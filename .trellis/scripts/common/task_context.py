...
def _validate_jsonl(jsonl_file: Path, repo_root: Path, task_dir: Path | None = None) -> int:
    ...
    for line in jsonl_file.read_text(encoding="utf-8").splitlines():
        ...
        if extension in _CODE_FILE_EXTENSIONS and not _is_exempt_from_code_file_warning(
            ...
            msg = f"{file_name}:{line_num}: Warning: {file_path} looks like a code file — "
            msg += "implement/check.jsonl should reference spec/research docs; "
            msg += "agents read code themselves"
            print(f"  {colored(msg, Colors.YELLOW)}")

        if max_file_bytes:
            size = full_path.stat().st_size
            if size > max_file_bytes:
                warn = f"{file_name}:{line_num}: Warning: {file_path} is {size} bytes, "
                warn += f"exceeds context_injection.max_file_bytes ({max_file_bytes}); "
                warn += "injection will truncate it"
                print(f"  {colored(warn, Colors.YELLOW)}")
