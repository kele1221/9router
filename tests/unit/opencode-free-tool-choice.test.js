...
// The OpenCode CLI wants its own tool quartet declared (anti-abuse fingerprint),
// so the executor appends bash/glob/grep/read after the caller's tools.
const FINGERPRINT_NAMES = ["bash", "glob", "grep", "read"];
const expectClientToolFirstWithFingerprint = (tools) => {
  expect(tools[0]).toEqual(TOOLS[0]);
  expect(tools.map((t) => t.name)).toEqual(
    expect.arrayContaining(["get_weather", ...FINGERPRINT_NAMES]),
  );
};
...
describe("opencode Free 1.3 tool_choice auto-only", () => {
  ...
  it.each([
    ...
    for (const model of [FREE_13, `${FREE_13}(max)`]) {
      ...
      expectClientToolFirstWithFingerprint(out.tools);
      expect(out.input).toEqual(INPUT);
    }
  });

  it("giữ auto; absent → auto (Responses default); tools/input nguyên vẹn", () => {
    ...
    expectClientToolFirstWithFingerprint(autoOut.tools);
    ...
    // Responses (flat) bodies default to "auto" in the fingerprint pass so the
    // appended decoy tools can never be selected implicitly.
    expect(absentOut.tool_choice).toBe("auto");
    expectClientToolFirstWithFingerprint(absentOut.tools);
  ...
  it("wire: execute gửi choice auto tới /zen/v1/responses", async () => {
    ...
    expectClientToolFirstWithFingerprint(sent.tools);
